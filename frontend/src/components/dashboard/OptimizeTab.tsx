...
export default function OptimizeTab({ onSelectProduct }: { onSelectProduct: (desc: string, title: string, image: string | null) => void }) {
  ...
  const [platform, setPlatform] = useState<'shopify' | 'woocommerce' | 'amazon'>('shopify');
    ...
    <div className="space-y-6 max-w-[1400px]">
      <div className="flex items-start justify-between mb-2">
        <div className="space-y-5">
          ...
          <div className="inline-flex bg-[#0f0f0f] border border-white/5 rounded-full p-1 mb-3 shadow-lg">
            <button
            ...
            <button
              ...
              className={`px-4 py-2 rounded-full text-sm font-medium transition-colors ${platform === 'amazon' ? 'bg-[#ff9900] text-white shadow-lg' : 'text-neutral-400 hover:text-white'}`}
            >
              Amazon
            </button>
          </div>
          ...
          {error.includes('not connected') ? (
            <div className="py-20 flex flex-col items-center justify-center text-center">
              <div className="w-16 h-16 rounded-full bg-white/5 border border-white/10 flex items-center justify-center mb-4 text-2xl">
                {platform === 'shopify' ? '🛍️' : platform === 'amazon' ? '📦' : '🛒'}
              </div>
              <h3 className="text-xl font-medium text-white mb-2">{platform === 'shopify' ? 'Shopify' : platform === 'amazon' ? 'Amazon' : 'WooCommerce'} not connected</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          ...
          {!loading && products.length === 0 && !error && (
            <div className="col-span-full py-24 text-center">
              ...
              <p className="text-neutral-500">There are no products in your {platform === 'shopify' ? 'Shopify' : platform === 'amazon' ? 'Amazon' : 'WooCommerce'} store.</p>
            </div>
          )}
        </div>
      )}
